...
enum class BuildSystem { None, CMake, Cargo, Npm, Make, Go };
...
static int build_cmake(bool release) {
  ...
  std::string configure =
      std::string("cmake -B build -DCMAKE_BUILD_TYPE=") + type;
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; ++i) {
    if (std::string_view(argv[i]) == "--release" ||
        std::string_view(argv[i]) == "-r") {
      release = true;
    }
  }
  ...
  if (bs == BuildSystem::None) {
    ...
    std::println(stderr, "  looked for: CMakeLists.txt, Cargo.toml, "
